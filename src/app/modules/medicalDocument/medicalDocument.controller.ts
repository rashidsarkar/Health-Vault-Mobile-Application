...
const upsertUserMedicalDocument = catchAsync(async (req, res) => {
  const { files } = req;
  ...
  const result = await medicalDocumentServices.upsertUserMedicalDocument(
    req.user.profileId,
    req.body,
  );
...
const getMyMedicalDocument = catchAsync(async (req, res) => {
  const result = await medicalDocumentServices.getMyMedicalDocument(
    req.user.profileId,
  );
